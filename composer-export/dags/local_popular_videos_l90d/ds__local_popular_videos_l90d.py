...
from local_popular_videos_l90d.clickhouse_utils import clickhouse_command
...
def ensure_clickhouse_table():
    clickhouse_command(
        """
        CREATE TABLE IF NOT EXISTS yral.local_popular_videos_l90d ON CLUSTER yral_cluster
        (
            `video_id` String,
            `region` String,
        ...
        ENGINE = ReplicatedMergeTree('/clickhouse/tables/{shard}/yral/local_popular_videos_l90d', '{replica}')
...
def create_local_popular_videos_l90d():
    ...
    clickhouse_command("TRUNCATE TABLE yral.local_popular_videos_l90d")
    clickhouse_command(
        """
        INSERT INTO yral.local_popular_videos_l90d
        (
            video_id,
            region,
        ...
        WITH
            stats AS (
                ...
                WHERE uvr.last_watched_timestamp >= now64(3) - INTERVAL 90 DAY
