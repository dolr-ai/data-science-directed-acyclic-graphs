"""
Last Fallback Cache Refresh DAG

This DAG triggers the recommendation-last-fallback Cloud Run service to refresh the zero-interaction fallback cache.
It creates an ephemeral Cloud Run job that scales to zero after completion.

The last fallback cache contains videos that have had zero user interactions in the last 90 days,
serving as an exploratory lever for the recommendation system to surface undiscovered content.
"""
...
DAG_ID = "cache_refresh_last_fallbacks"
...
GCP_CONN_ID = os.environ.get("RECSYS_GCP_CONN_ID", "google_cloud_default")
...
SERVICE_NAME = "recommendation-last-fallback"
IMAGE_NAME = "recommendation-last-fallback"  # Matches the image name in GitHub workflow
REPOSITORY = "recsys-repository"  # Hardcoded to match GitHub workflow

# Status variable name
STATUS_VARIABLE = "cache_refresh_last_fallback_completed"
...
class GoogleChatAlert:
    ...
    def __init__(self, webhook_url=None, logo_url=None, project_id=None, dag_id=None):
        ...
        self.logo_url = (
            logo_url or "https://placehold.co/400/FF6600/FFFFFF.png?text=ZZ&font=roboto"
        )
    ...
    def send(self, context, status):
        ...
        card = {
            "cards": [
                {
                    "header": {
                        "title": f"Last Fallback Cache Alert: {config['title']}",
                        "subtitle": f"{dag_id}",
...
def generate_job_name(**kwargs):
    ...
    if execution_date:
        # Format: recommendation-last-fallback-job-YYYYMMDD-HHMMSS
    ...
    job_name = f"rs-last-fallback-cache-update-{timestamp}"
...
def initialize_status_variable(**kwargs):
    """Initialize the cache_refresh_last_fallbacks_completed status variable to False."""
    try:
...
def set_status_completed(**kwargs):
    """Set the cache_refresh_last_fallbacks_completed status variable to True."""
    try:
...
with DAG(
    ...
    if SERVICE_ACCOUNT:
        job_config["template"]["template"]["service_account"] = SERVICE_ACCOUNT
    ...
    run_job = CloudRunExecuteJobOperator(
        task_id="task-run_last_fallbacks_refresh",
        project_id=PROJECT_ID,
        region=REGION,
        job_name=job_name,
        gcp_conn_id=GCP_CONN_ID,
