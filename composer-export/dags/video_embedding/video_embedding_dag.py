...
GCP_CONN_ID = os.environ.get("VIDEO_EMBED_BIGQUERY_CONN_ID", "bigquery_default")


def send_alert_to_google_chat(_context):
    if not GOOGLE_CHAT_WEBHOOK:
        return

    message = {"text": "DAG video_embed_pipeline_dag failed."}
    try:
        requests.post(GOOGLE_CHAT_WEBHOOK, json=message, timeout=10)
    except Exception:
        # Alerts must not create a second failure path.
        pass
